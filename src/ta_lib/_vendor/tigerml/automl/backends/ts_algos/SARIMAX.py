...
class ARIMAXRelated(TimeSeriesModels):
    """Class for Timeseries ARIMAX model related functions. Uses statsmodels package.
    ...
    def __init__(self, **kwargs):
        """The initialisation for ARIMAXRelated with kwargs."""

        super().__init__(**kwargs)

        self.algo_choice_dict = {"SARIMAX": SARIMAX}

        master_kwargs_sarimax = {
            "fit": get_default_args(SARIMAX.fit),
            "__init__": get_default_args(SARIMAX.__init__),
        }

        self.kwargs_choice_dict = {"SARIMAX": master_kwargs_sarimax}

    def runModel(self, model_data: pd.DataFrame, model_param_dict: dict):
        """
        Runs the model for ARIMAXRelated  class on input data and parameters.
    ...
    def getPredictions(self, model_data: pd.DataFrame, model_object):
        """
        Generates predictions/scoring using the input model object on the input data.
        ...
        model_data_w_pred : pd.DataFrame
            Input data with added column for predictions.
        """
    ...
    def GridSearchCV(self, model_related_dict: dict):
        ...
        model_related_dict : dict
            Dictionary of parameters for the models with keys as 'validation' and 'param_dict'.
